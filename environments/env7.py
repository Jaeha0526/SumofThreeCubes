...
class SumofCubesEnv7(SumofCubesEnv):
        ...
        if dones is not None :
            for i in range(self.num_envs):
                if dones[i]:
                    ...
                    # reset done env
                    self.states[i] = np.random.randint(-3, 3, (5,))
    ...
    def single_reward(self,state):
        ...
        if fs in self.solutions:
            self.solutions[fs] += 1
            return (self.rewards[0]*(1 - k/self.max_k) + self.rewards[3]*(x**2 + y**2 + z**2)
                      + self.rewards[5]*(w**2 + v**2)
                    ) * 2**(-self.solutions[fs]/self.depreciation) + self.rewards[4]

        # new solution
        self.solutions[fs] = 1
        reward = self.rewards[2]*(1 - k/self.max_k) + self.rewards[3]*(x**2 + y**2 + z**2) + self.rewards[5]*(w**2 + v**2)

        return reward


    def single_action(self, action): # -1 ~ +1
        # if action == 4 :
        #    action += 1
        return np.array([(action // 192) + 1 , (action % 192)//64 + 1 , (action % 64)//16 - 1 , (action % 16)//4 - 1, (action % 4) - 1])
